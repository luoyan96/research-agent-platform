...
> 当前处于产品规划与工程起步阶段。仓库已有本地项目/成果存储核心、十项科研 Skills、评测材料和待联调的 Harness 适配源码。F0 新增三页前端演示；团队账号、真实任务服务、匹配分配、权限、记忆和后台调度仍未接通。

**F0 前端预览**：需求入口、实验室总览和任务详情的启动与边界见 [apps/web](apps/web/README.md)。运行 `pnpm --filter @research-agent/web dev:demo --port 4173`；所有示例标注“演示数据 · 尚未连接服务”。新增应用已纳入根 CI，不代表真实协作或模型功能完成。
...
| F0 三页网页预览 | 已实现演示 | 显式 fixture 模式，无真实任务服务 |
| 任务分配、成员及能力权限、长期记忆、后台调度 | 规划中 | 尚未实现或部署 |
...
`pnpm run ci` 检查文档和 Skills 引用，构建基础包、共享契约和前端，执行类型检查、契约一致性、单元测试与前端生产隔离检查。`pnpm demo` 在系统临时目录创建演示项目、保存一份计划、通过新存储实例重新读取，并列出十项 Skills。演示打印文件位置，便于查看；不调用模型，不需要 API key。
...
F0 提供明确标识的三页视觉演示，真实平台服务将在对应里程碑接通。
